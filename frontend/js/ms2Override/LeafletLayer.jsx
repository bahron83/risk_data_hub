...
class LeafletLayer extends React.Component {
    ...
    componentDidMount() {         
        ...
        if(options.pane !== undefined) {
            const paneName = options.pane;
            const zIndex = options.zIndex || 1;
            this.createPane(paneName);
            this.props.map.getPane(paneName).style.zIndex = zIndex;
        }                            
        this.createLayer(this.props.type, options, this.props.position, this.props.securityToken);
    ...
    updateZIndex = (position) => {                
        let newPosition = position || this.props.options.zIndex || this.props.position;                
    ...
    createPane = (name) => {
        const pane = this.props.map.getPane(name);
        if(pane === undefined)
            this.props.map.createPane(name);
    }
