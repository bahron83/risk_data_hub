...
Since this application uses geonode, base source of settings is ``geonode.settings`` module. It provides defaults for many items, which are used by geonode. This application has own settings module, ``risk_data_hub.settings``, which includes ``geonode.settings``. It customizes few elements:
 ...
 * Adds ``risk_data_hub`` to installed applications, updates templates, staticfiles dirs, sets urlconf to ``risk_data_hub.urls``.

Whether you deploy development or production environment, you should create additional settings file. Convention is to make ``risk_data_hub.local_settings`` module. It is recommended to use ``risk_data_hub/local_settings.py``.. That file contains small subset of settings for edition. It should:
...
::
    ...
    |   geonode.settings     |     ->      |  risk_data_hub.settings    |      ->     |  risk_data_hub.local_settings |
    +------------------------|-------------|-------------------------------|-------------|----------------------------------+
